import {
  ...
  (async () => {
    ...
    await createSwig(connection, swigId, rootAuthority, rootActions, rootKeypair.publicKey, [rootKeypair]);
    ...
    for (const { name, amount } of rolesToCreate) {
      ...
      const roleAuthority = Ed25519Authority.fromPublicKey(roleKeypair.publicKey);
      ...
      console.log(`[${name}] Added role with tx: https://explorer.solana.com/tx/${tx}?cluster=custom`);
